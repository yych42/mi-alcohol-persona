...
class PushToHub(GlobalStep):
    ...
    Attributes:
        ...
        token: The token that will be used to authenticate in the Hub. If not provided, the
            ...
            to `None`.
        dropna: Whether to drop rows where any column contains empty values. Defaults to
            `False`.
    ...
    Runtime parameters:
        ...
        - `dropna`: Whether to drop rows with empty values in any column.
    ...
    Examples:
        ...
        push = PushToHub(repo_id="path_to/repo", dropna=True)
        push.load()
    ...
    dropna: RuntimeParameter[bool] = Field(
        default=False,
        description="Whether to drop rows where any column contains empty values. Defaults"
        " to `False`.",
    )
    ...
    def process(self, inputs: StepInput) -> "StepOutput":  # type: ignore
        ...
        # First pass: collect all data and check for empty values if dropna is True
        for input in inputs:
            if self.dropna:
                # Check if any value in the input dictionary is empty (None, "", or whitespace)
                has_empty = any(
                    not str(value).strip() if value is not None else True
